...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    const user = await sb.insert(db, "users", {
      username, display_name: displayName, password_hash: hash, salt, role, email,
    });
    ...
    return new Response(
      JSON.stringify({
        ok: true,
        user: { id: user.id, username, display_name: displayName, role, email },
        first_admin: isFirst,
      }),
      {
        status: 200,
